...
def load_args():
    ...
    arg_parse.add_argument("-f", "--filename", type=str, required=True,
                           help="Filename to generate gant")
    return arg_parse.parse_args()

def genGant(filename, sep = ','):
    data = pd.read_csv(filename, sep=sep)#здесь ваш файл с расширением .csv
    ...
    ax.grid(True)
    fig.savefig(f'{filename}.svg', format='svg')#с таким названием будет сохранен итог в формате .svg

if __name__ == "__main__":
    args = load_args()
    genGant(filename = args.filename)